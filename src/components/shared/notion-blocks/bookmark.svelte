<script lang="ts">
	...
	onMount(async () => {
		if (typeof window !== 'undefined') {
			...
			try {
				if (!isAmazonURL(url)) {
					const a = metascraper([titleRule(), descriptionRule(), imageRule()]);
					const html = urlMap[urlString!];
					if (html) {
						metadata = await a({ html, url: urlString! });
					}
				}
			} catch (err) {
				console.log(err);
				...
				<a href={url.toString()} target="_blank" rel="noopener noreferrer">
					...
					<div>
						{#if metadata && metadata.image}
							<img src={metadata.image} alt="Site image of the bookmark" loading="lazy" />
						{/if}
					</div>
				</a>
			</div>
